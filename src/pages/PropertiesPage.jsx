...
const getProjectQuickActions = (property) => {
  ...
  if (isActive && hasAcquisitions) {
    return [
      { label: "Open Home", to: buildPropertyWorkspacePath(propertyKey, "home"), primary: true },
      { label: "Execution", to: buildPropertyWorkspacePath(propertyKey, "execution") },
      { label: "Budget", to: buildPropertyWorkspacePath(propertyKey, "budget") },
      { label: "Deal", to: buildPropertyWorkspacePath(propertyKey, "deal") },
    ];
  }

  if (hasAcquisitions && hasPipeline) {
    return [
      { label: "Open Budget", to: buildPropertyWorkspacePath(propertyKey, "budget"), primary: true },
      { label: "Deal", to: buildPropertyWorkspacePath(propertyKey, "deal") },
  ...
  if (hasPipeline) {
    return [
      { label: "Open Deal", to: buildPropertyWorkspacePath(propertyKey, "deal"), primary: true },
      { label: "Project setup", to: buildPropertyWorkspacePath(propertyKey, "settings") },
    ];
  }

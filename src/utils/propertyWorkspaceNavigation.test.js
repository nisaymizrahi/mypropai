...
describe("propertyWorkspaceNavigation", () => {
  ...
  it("builds section hash links for the new workspace sub-navigation", () => {
    expect(buildPropertyWorkspaceSectionPath("abc-123", "budget", "expenses")).toBe(
      "/properties/abc-123/budget#expenses"
    );
  ...
  it("normalizes invalid section ids back to each tab default", () => {
    expect(getDefaultPropertyWorkspaceSectionId("home")).toBe("today");
    expect(normalizePropertyWorkspaceSectionId("budget", "missing")).toBe("snapshot");
    expect(normalizePropertyWorkspaceSectionId("execution", "vendors")).toBe("vendors");
  });
});

...
export const PAYMENT_COST_CLASS_OPTIONS = [
...
export const PAYMENT_COST_TYPE_OPTIONS_BY_CLASS = {
  construction: [
...
const PAYMENT_COST_CLASS_SET = new Set(PAYMENT_COST_CLASS_OPTIONS.map((option) => option.value));

const PAYMENT_COST_TYPE_LABELS = Object.values(PAYMENT_COST_TYPE_OPTIONS_BY_CLASS).flat().reduce(
  (accumulator, option) => ({
    ...accumulator,
...
export const normalizePaymentCostClass = (value = "") => {
  const normalizedValue = normalizeKey(value);
  return PAYMENT_COST_CLASS_SET.has(normalizedValue) ? normalizedValue : "";
};

export const getPaymentCostClassLabel = (value = "") =>
  PAYMENT_COST_CLASS_OPTIONS.find((option) => option.value === value)?.label || "General";

export const getPaymentCostTypeOptions = (costClass = "") =>
  PAYMENT_COST_TYPE_OPTIONS_BY_CLASS[normalizePaymentCostClass(costClass)] || [];

export const normalizePaymentCostType = (costClass = "", value = "") => {
...
export const getPaymentCostTypeLabel = (value = "") =>
  PAYMENT_COST_TYPE_LABELS[normalizeKey(value)] || "";

export const inferPaymentCostClass = ({
  title = "",
  description = "",
  recurringCategory = "",
  budgetItem = null,
...
export const inferPaymentCostType = ({
  costClass = "",
  title = "",
  description = "",
  recurringCategory = "",
  ...
  if (normalizedCostClass === "holding") {
    ...
    if (recurringKey === "utilities" || containsAnyTerm(haystack, ["utility", "electric", "water", "gas"])) {
      return "utilities";
    }
...
export const resolvePaymentClassification = (input = {}) => {
...
export const getPaymentScopeExpectation = (costClass = "") =>
  normalizePaymentCostClass(costClass) === "construction"
    ? "Recommended to tie to a scope item"
    : "Scope link is optional";

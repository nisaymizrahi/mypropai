...
const RentalCalculator = () => {
    ...
    const calculations = useMemo(() => {
        ...
        const mortgagePayment = numberOfPayments > 0 && monthlyInterestRate > 0 ? loanAmount * (monthlyInterestRate * Math.pow(1 + monthlyInterestRate, numberOfPayments)) / (Math.pow(1 + monthlyInterestRate, numberOfPayments) - 1) : 0;
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-sm border">
            <h3 className="text-xl font-semibold text-gray-800 mb-6 border-b pb-4">Rental Property Calculator</h3>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-gray-50 p-6 rounded-lg border">
                    ...
                    <div className="mt-8 border-t pt-4 space-y-3">
                        <div className="flex justify-between text-base"><span className="font-semibold">Cash on Cash ROI</span><span className={`font-bold ${calculations.cashOnCashROI >= 0 ? 'text-green-600' : 'text-red-600'}`}>{calculations.cashOnCashROI.toFixed(1)}%</span></div>
                         <div className="flex justify-between text-base"><span className="font-semibold">Cap Rate</span><span className={`font-bold ${calculations.capRate >= 0 ? 'text-green-600' : 'text-red-600'}`}>{calculations.capRate.toFixed(1)}%</span></div>
                    </div>
                </div>
            </div>
        </div>

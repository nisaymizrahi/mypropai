...
const DashboardTab = ({ investment, budgetItems, expenses, tasks }) => {
    ...
    const upcomingTasks = useMemo(() => {
        return tasks
            ...
            .slice(0, 3); // Get the next 3 upcoming tasks
    }, [tasks]);
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-white p-6 rounded-lg shadow-sm border border-brand-gray-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <StatCard title="Remaining" value={`$${financialSummary.remainingBudget.toLocaleString()}`} colorClass={financialSummary.remainingBudget >= 0 ? 'text-green-600' : 'text-red-600'} />
                    </div>
                    <div className="mt-4">
                    ...
                    <h3 className="text-xl font-semibold text-brand-gray-800 mb-4">Schedule Overview</h3>
                     <div className="mt-4">
                        <div className="flex justify-between text-sm mb-1">
                            ...
                            <span className="font-semibold">{scheduleSummary.completedTasks} of {scheduleSummary.totalTasks}</span>
                        </div>
                    ...
                    <div className="space-y-3">
                        {upcomingTasks.length > 0 ? upcomingTasks.map(task => (
                            <div key={task._id} className="text-sm border-b pb-2">
                                ...
                                <p className="text-xs text-brand-gray-500">Due: {new Date(task.endDate).toLocaleDateString()}</p>
                            </div>
                        )) : (
